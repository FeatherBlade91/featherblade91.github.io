...
const SaturnSky = {
  ...
  init(stage) {
    ...
    /* 双星星体：自发光（不做光照），但有立体感——边缘变暗（limb darkening，
     * 真实恒星的临边昏暗）+ 程序化米粒组织（fbm 噪声在星面方向上采样，缓慢
     * 翻腾，像等离子体对流胞）+ 一圈同色系色球层辉边。uBright 带心跳脉冲与
     * 异相闪烁。模型矩阵只有平移+均匀缩放，法线方向不变：vNor 直接取球面
     * 方向作物体空间坐标，纹理牢牢钉在星面上（星体本身不自转，不会滑）。 */
    this.starBodyProg = this.makeProgram(
      ...
      `precision mediump float;
       ...
       void main() {
         vec3 dir = normalize(vNor); // 物体空间方向：纹理坐标，星怎么转都跟面走
         float facing = max(dot(normalize(vNorV), normalize(-vPosV)), 0.0);
         // 米粒组织：低频起伏 + 一层细节，时间项让它极缓慢地翻腾
         float cells = fbm(dir * 4.0 + vec3(0.0, uTime * 0.04, uTime * 0.03));
         float fine  = fbm(dir * 9.0 - vec3(uTime * 0.02, 0.0, 0.0));
         float gran = smoothstep(0.30, 0.78, cells * 0.72 + fine * 0.28);
         // 临边昏暗：星心炽亮，向边缘按 pow 曲线压暗——球体立体感的主要来源
         float limb = pow(facing, 0.55);
         vec3 col = uCore * (0.50 + 0.62 * limb);
         // 亮胞更亮更白（向色温高位推）、暗隙略沉，越靠星心对比越强
         col *= 0.86 + 0.42 * gran;
         col += uCore * gran * limb * 0.35;
         // 色球层辉边：掠射角一圈同色系的亮晕，把星体从背景里托出来
  ...
  render(t, cam) {
    ...
    gl.uniform1f(this.starBodyProg.u.uTime, t); // 驱动星面米粒组织缓慢翻腾
